...
figMW <- function(filename = "img/figMW", part = 1, hg = 5, wi = 8.5, res = 300,
  ...
  png(filename, height = hg, width = wi, unit="in", res = res)
    ...
    if (part>3){
      ...
      mtext(3, at=-0.5, text="Rates", cex=cex.l, line=.1)
      box(bty="l")
      ...
      if (part>4){
        if (part<6){
          legend(5, 0.5, c("Colonisation"), lty=1, ncol=2, bty="n",
          seg.len=1, cex=2.4)
        } else {
          legend(5, 0.5, c("Colonisation", "Extinction"), lty=c(1,4), ncol=2, bty="n",
          seg.len=1, cex=2.4)
        }
      }
    }
